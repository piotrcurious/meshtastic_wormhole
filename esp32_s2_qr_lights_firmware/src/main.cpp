...
// Dedicated ESP32-S2 QR Lights Demo Node Firmware
...
void on_mesh_rx(const uint8_t* payload, size_t length, uint64_t from_node) {
    ...
    if (!WormholePacket::unpack(payload, length, pkt)) {
        Serial.println("GPS LED Node WiFi RX Error: failed to unpack wormhole packet");
        return;
    }

    Serial.printf("GPS LED Node WiFi RX: received packet %d from source %llx\n", pkt.packet_id, pkt.source_id);
    ...
    if (parse_meshtastic_header(pkt.payload.data(), pkt.payload.size(), to_node, from_node_mesh)) {
        ...
        Serial.printf("GPS LED Node Smart Routing: Mapped Meshtastic Node %x to Wormhole %llx\n", from_node_mesh, pkt.source_id);
    }

    // 1. Loop prevention check
    if (pkt.has_visited(esp_node_id)) {
        Serial.printf("GPS LED Node Loop Prevention: Dropped packet %d - already visited this node\n", pkt.packet_id);
        return;
    }
    ...
    if (is_targeted && pkt.hops.size() >= 2) {
        ...
        if (target_id != esp_node_id && pkt.source_id != esp_node_id) {
            Serial.printf("GPS LED Node Smart Routing: Bypassing packet %d - targeted for Wormhole %llx\n", pkt.packet_id, target_id);
            return;
        }
    }
    ...
    if (dedup.is_duplicate(pkt.source_id, pkt.packet_id)) {
        Serial.printf("GPS LED Node Deduplication: Dropped packet %d - already processed\n", pkt.packet_id);
        return;
    }
    ...
    if (sent) {
        Serial.printf("GPS LED Node Mesh RX: Successfully repeated/extended packet %d\n", pkt.packet_id);
    } else {
        Serial.println("GPS LED Node Mesh Error: failed to repeat/extend packet");
    }
}
